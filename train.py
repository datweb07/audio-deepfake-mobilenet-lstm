...
def restore_checkpoint_weights(model: tf.keras.Model) -> None:
    """Restore the globally best validation checkpoint without replacing layer references."""
    if not os.path.isfile(config.TRAINING_CHECKPOINT_PATH):
        raise RuntimeError("Training produced no validation checkpoint")
    selected_model = tf.keras.models.load_model(config.TRAINING_CHECKPOINT_PATH, compile=False)
...
def main() -> None:
    ...
    restore_checkpoint_weights(model)
