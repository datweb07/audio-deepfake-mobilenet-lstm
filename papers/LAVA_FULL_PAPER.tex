...
\maketitle
\begin{abstract}
LAVA evaluates heterogeneous detectors through a common integrity, score, robustness, and timing contract while preserving native architectures. This study evaluates four LAVA-trained Mel-sequence models and externally pretrained RawNet2 and AASIST checkpoints. Clean evaluation used all 2,737 canonical test recordings. ShuffleNetV2-LSTM obtained the best F1 (0.9824), AUC (0.9929), and EER (1.46\%); MobileNetV3-LSTM had the lowest desktop-CPU latency (43.8 ms), while ShuffleNet required 62.5 ms (RTF 0.0208). A fixed 100-recording diagnostic showed small codec degradation but strong low-SNR sensitivity; ShuffleNet's mean degradation across nine conditions was 0.1623. The diagnostic Pareto set contains MobileNet, ShuffleNet, and AASIST. Heterogeneous provenance precludes architecture-only conclusions. Physical replay, unseen data, edge devices, multiple seeds, and full-test robustness remain future work.
\end{abstract}
...
The current repository supports a six-detector clean study: MobileNetV3Small-LSTM, EfficientNet-B0-LSTM, MnasNet-A1-LSTM, and ShuffleNetV2-LSTM were trained locally with unequal initialization histories; RawNet2 and AASIST use external checkpoints. We ask: \emph{RQ1}, how do the lightweight artifacts compare with external references under one clean protocol? \emph{RQ2}, how do scores change under the executed stress conditions? \emph{RQ3}, which systems are non-dominated in detection--robustness--runtime space?

\section{Related Work}
ASVspoof covers synthetic, converted, and replayed speech \cite{todisco2019asvspoof}; WaveFake was not evaluated here \cite{frank2021wavefake}. RawNet2 learns from waveforms \cite{tak2021rawnet2}; AASIST integrates heterogeneous graph attention \cite{jung2022aasist}. MobileNetV3 \cite{howard2019mobilenetv3}, EfficientNet \cite{tan2019efficientnet}, MnasNet \cite{tan2019mnasnet}, and ShuffleNetV2 \cite{ma2018shufflenetv2} provide complementary compact backbones.
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{lava_6_model_overview.png}\caption{LAVA six-detector architecture and evaluation boundary.}\label{fig:overview}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{lava_benchmark_pipeline.png}\caption{Reproducible benchmark pipeline.}\label{fig:pipeline}\end{figure}
...
ShuffleNetV2-LSTM&Light&TF 2.15&Mel, 3 s&1024-D backbone + LSTM&1,868,441&LAVA, scratch, early-stopped best\\
...
Validation-F1 thresholds are 0.82/0.90/0.90/0.12 for MobileNet/EfficientNet/MnasNet/ShuffleNet. References retain uncalibrated 0.5 defaults. Test labels select neither weights nor thresholds.
...
All six artifacts passed load, score-bound, hash, and adapter-parity checks. Table~\ref{tab:clean} and Figs.~\ref{fig:roc}--\ref{fig:det} use all 2,737 test samples.
...
ShuffleNet&.9850&.9795&.9854&.9824&.9847&.9929&.0146&&1551/24/17/1145\\
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{roc_comparison_6_models.png}\caption{Full-test ROC curves.}\label{fig:roc}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{det_comparison_6_models.png}\caption{Full-test DET curves.}\label{fig:det}\end{figure}
The four local lightweight artifacts exceed the external references under this contract. ShuffleNet leads clean aggregates; MobileNet is fastest. Different provenance and initialization prohibit an architecture-only ranking.
...
\begin{tabular}{lrrrr}\toprule Model&Noise&Codec&Replay&All 9\\\midrule MobileNet&.6722&.0095&.0620&.3099\\EfficientNet&.8053&.0000&.0641&.3650\\MnasNet&.6119&.0233&.1489&.2989\\ShuffleNet&.3555&.0032&.0256&.1623\\RawNet2&.5077&-.0007&-.0109&.2241\\AASIST&-.0660&-.0074&-.0682&-.0402\\\bottomrule\end{tabular}\end{table}
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{noise_f1_vs_snr_6_models.png}\caption{Diagnostic F1 under AWGN; not full-test robustness.}\label{fig:noise}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{robustness_heatmap_6_models.png}\caption{Per-condition diagnostic $\Delta F1$.}\label{fig:heat}\end{figure}
...
MobileNet&5.64&554.6&13.43&30.07&43.81/52.12&22.83&.0146\\EfficientNet&18.96&651.2&13.59&168.36&175.85/196.11&5.69&.0586\\MnasNet&13.43&566.0&13.60&95.25&117.00/138.96&8.55&.0390\\ShuffleNet&7.74&501.8&15.57&48.05&62.52/75.37&15.99&.0208\\RawNet2&67.65&249.5&.63&95.10&96.54/102.62&10.36&.0239\\AASIST&1.61&442.5&.47&359.61&362.91/398.45&2.76&.0899\\\bottomrule\end{tabular}\end{table*}
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{end_to_end_latency_bar_6_models.png}\caption{Measured end-to-end CPU latency.}\label{fig:lat}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{parameters_bar_6_models.png}\caption{Parameter count under the documented Keras/native conventions.}\label{fig:params}\end{figure}

\subsection{Exploratory Pareto analysis (RQ3)}
Table~\ref{tab:pareto} reports the three diagnostic objectives. MobileNet, ShuffleNet, and AASIST are non-dominated; ShuffleNet dominates MnasNet and RawNet2, while MobileNet dominates EfficientNet. Official full-test Pareto is NOT\_RUN.
...
MnasNet&.0476&.2989&.0390&No\\
ShuffleNet&.0476&.1623&.0208&Yes\\
RawNet2&.4138&.2241&.0239&No\\
AASIST&.3810&-.0402&.0899&Yes\\\bottomrule
\end{tabular}\end{table}
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{pareto_eer_rtf_6_models.png}\caption{Diagnostic EER--RTF projection; stars denote the three-objective frontier.}\label{fig:pareto}\end{figure}

\subsection{Errors and uncertainty}
All six models were correct on 911 samples and wrong on 10. The four lightweight systems were unanimously correct while both references were wrong on 785 samples; the reverse occurred on three. Figure~\ref{fig:agree} shows the matrix.
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{agreement_heatmap_6_models.png}\caption{Full-test pairwise prediction agreement.}\label{fig:agree}\end{figure}
...
\begin{table*}[t]\caption{Preserved five-model full-test intervals and appended ShuffleNet diagnostic interval (1,000 replicates).}\label{tab:ci}\centering\small
...
AASIST&[.4928,.5349]&[.5377,.5815]&[.4279,.4660]\\
ShuffleNet$^\dagger$&[.9367,1.0000]&[.9006,1.0000]&[.0000,.1190]\\\bottomrule
\end{tabular}\end{table*}

\section{Discussion}
RQ1 shows strong clean performance for local lightweight models, with ShuffleNet providing the best clean detection and MobileNet the lowest runtime. This does not contradict reference publications because domain, adapter and threshold policies differ. RQ2 shows codec resilience but substantial low-SNR sensitivity. RQ3 demonstrates that Pareto membership is not a scalar ranking.

\section{Limitations}
The scope contains six clean models, but robustness covers only 100 diagnostic samples. Provenance is heterogeneous; EfficientNet is warm-up-only, MnasNet/ShuffleNet are scratch-trained, and RawNet2/AASIST are external. Unknown speaker/source/generator metadata limits the split to checksum-group-disjoint. Replay is simulated, no unseen dataset exists, and timing covers one Windows desktop CPU rather than edge hardware.

\section{Conclusion}
LAVA provides a traceable evaluation boundary across six heterogeneous anti-spoofing models. ShuffleNet obtained the best full-clean detection aggregate and MobileNet the best CPU latency. Diagnostic stress suggests codec tolerance but white-noise fragility. Full-test stress, physical replay, unseen data, edge devices, and multiple seeds remain future work.
...
Canonical manifests are in \texttt{data/manifests}; historical results remain in \texttt{outputs/lava\_5}; ShuffleNet-only measurements and six-model aggregates are in \texttt{outputs/lava\_6}. The incremental runner and report generator verify all values without invoking training.
