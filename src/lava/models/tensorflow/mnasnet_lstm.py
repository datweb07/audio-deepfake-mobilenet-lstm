...
class MnasNetLSTMDetector(KerasLightweightDetector):
    def __init__(self) -> None:
        ...
        self.inference_batch_size = int(os.getenv("LAVA_MNASNET_INFERENCE_BATCH_SIZE", "1"))
        if self.inference_batch_size < 1:
            raise ValueError("MnasNet inference batch size must be positive")
    ...
    def predict_scores(self, audio_paths: Sequence[str]) -> np.ndarray:
        ...
        for start in range(0, len(audio_paths), self.inference_batch_size):
            ...
            if len(audio_paths) > 128 and start % 128 == 0:
                print(f"MnasNet inference {min(start + len(paths), len(audio_paths))}/{len(audio_paths)}", flush=True)
        return validate_p_fake(scores).astype(np.float32)
