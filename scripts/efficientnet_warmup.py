"""EfficientNet-only, offline Keras 3 -> Keras 2.15 deployment preparation.
...
WORK = ROOT / "outputs/efficientnet_conversion"
SOURCE = ROOT / "efficientnet_b0_warmup_checkpoint/warmup_best.keras"
...
def export_source():
    ...
    if model.name != "efficientnet_b0_lstm_audio_deepfake":
        raise ValueError("Not the expected EfficientNet checkpoint")
    groups, arrays = {}, {}
    ...
    embedding_model = keras.Model(model.inputs, model.get_layer("time_distributed_efficientnet_b0").output)
...
def rebuild_graph(description):
    ...
    allowed = {"Rescaling", "Normalization", "ZeroPadding2D", "Conv2D", "BatchNormalization", "Activation", "DepthwiseConv2D", "GlobalAveragePooling2D", "Reshape", "Multiply", "Dropout", "Add", "LSTM", "Dense"}
...
def convert():
    ...
    EfficientNetB0LSTMDetector().validate_model(model)
    inputs = np.load(WORK / "parity_inputs.npy")
    expected = np.load(WORK / "source_predictions.npz")
    embedding_model = tf.keras.Model(model.inputs, model.get_layer("time_distributed_efficientnet_b0").output)
...
def publish():
    ...
    from src.lava.data.loader import load_split
    from src.lava.data.split import stable_manifest_hash, validate_split_records
    from src.lava.models.tensorflow.efficientnet_b0_lstm import EfficientNetB0LSTMDetector
    from src.metrics import calibrate_threshold
    from src.preprocessing import process_audio_file
    report = read_json(WORK / "conversion_report.json")
    converted = WORK / "converted.keras"
    if report["status"] != "PASS" or report["converted_sha256"] != sha(converted) or report["source_sha256"] != sha(SOURCE):
        raise ValueError("Conversion verification missing or stale")
    state = read_json(LIFECYCLE)
    with (ROOT / "data/manifests/split_manifest.csv").open(encoding="utf-8", newline="") as f:
        records = list(csv.DictReader(f))
    with (ROOT / "data/manifests/dataset_manifest.csv").open(encoding="utf-8", newline="") as f:
        inventory = list(csv.DictReader(f))
    for row in inventory:
        row["included"] = row["included"].lower() == "true"
    validate_split_records(inventory, records)
    manifest_hash = stable_manifest_hash(records)
    if manifest_hash != state["training_manifest_hash"]:
        raise ValueError("Training/current manifest mismatch")
    for row in records:
        if row["split"] in {"validation", "test"}:
            ...
            if sha(path) != row["sha256"]:
                raise ValueError(f"Audio changed since manifest: {path}")
    detector = EfficientNetB0LSTMDetector()
    if any(p.exists() for p in (detector.spec.model_artifact, detector.spec.threshold_artifact, detector.spec.metadata_artifact)):
        raise FileExistsError("Refusing to overwrite an existing EfficientNet deployment")
    ...
    for start in range(0, len(paths), 128):
        probabilities.extend(detector.predict_scores(paths[start:start+128]).tolist())
        print(f"Validation calibration {min(start+128,len(paths))}/{len(paths)}", flush=True)
    if not np.all(np.isfinite(probabilities)) or np.min(probabilities) < 0 or np.max(probabilities) > 1:
        raise ValueError("Invalid P(FAKE) during validation calibration")
    threshold, f1 = calibrate_threshold(labels, probabilities)
    val_rows = [r for r in records if r["split"] == "validation"]
    with (WORK / "validation_scores.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["sample_id", "label", "p_fake"])
        writer.writerows((r["sample_id"], y, p) for r,y,p in zip(val_rows,labels,probabilities))
    spec = detector.spec
    metadata = {"detector_name": spec.name, "display_name": spec.display_name, "framework": "tensorflow", "framework_version": tf.__version__, "group": "lightweight", "architecture": "6 chronological Mel images -> TimeDistributed(EfficientNetB0) -> LSTM(128) -> Dense(64,relu) -> Dropout(0.4) -> sigmoid", "initialization": "imagenet_pretrained", "pretraining_status": "VERIFIED_IMAGENET", "label_mapping": {"REAL": 0, "FAKE": 1}, "score_semantics": "sigmoid output = P(FAKE)", "parameter_count": detector.model.count_params(), "serialized_size": converted.stat().st_size, "training_manifest_hash": manifest_hash, "training_seed": state["seed"], "input_contract": {"shape": [6,224,224,3], "sample_rate": 22050, "audio_duration": 3.0, "input_scale": [0,255]}, "selection": {"stage": "warmup", "best_epoch": state['warmup_best_epoch'], "best_value": state['warmup_best_val_loss'], "monitor": "val_loss", "test_used": False, "global_selection_finalized": False, "deployment_choice": "user_selected_best_available_warmup"}, "final_threshold": threshold, "threshold_source": "validation FAKE-class F1", "threshold_validation_f1": f1, "calibration_samples": len(labels), "load_smoke_test": "PASS", "conversion": report, "limitations": ["Fine-tune final model unavailable; this is a warm-up-only deployment", "Kaggle preprocessing parity cannot be independently established from checkpoint alone", "Supplied root metadata/threshold belong to MobileNet and were not reused"]}
    metadata["inference"] = {"default_batch_size": 1, "calibration_batch_size": detector.inference_batch_size, "training": False, "precision": "float32"}
    spec.model_artifact.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(converted, spec.model_artifact)
    detector.load()
    write_json_atomic(spec.threshold_artifact, {"threshold": threshold, "source": "validation", "metric": "FAKE-class F1", "manifest_hash": manifest_hash, "samples": len(labels)})
    write_json_atomic(spec.metadata_artifact, metadata)
    assert load_threshold(spec) == threshold
    write_json(WORK / "calibration_report.json", {"threshold": threshold, "validation_f1": f1, "samples": len(labels), "manifest_hash": manifest_hash})
    print(f"Published EfficientNet warm-up; validation threshold={threshold}, F1={f1}", flush=True)
